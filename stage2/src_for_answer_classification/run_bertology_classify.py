...
class MyDataset(Dataset):
    ...
    def format_one_data(self, p1, p2):
        ...
        input_tokens = ['[CLS]'] + tokens1
        if p2 is not None:
            tokens2 = self.tokenizer.tokenize(p2)
            input_tokens += ['[SEP]'] + tokens2
        input_tokens = input_tokens[:self.max_len - 1] + ['[SEP]']
    ...
    def format_one_data_roberta(self, p1, p2):
        input_ids = self.tokenizer.encode(p1, p2, max_length=self.max_len, pad_to_max_length=True, truncation=True)
    ...
    def load_pair_datas(self, df, model_type):
        for p0, p1, p2, label in tqdm(df):
            p0 = p0.lower()
            p1 = p1.lower()
            p2 = p2.lower()
            p = p0 + p1 + p2
            if 'roberta' in model_type:
                data = self.format_one_data_roberta(p, None)
            else:
                data = self.format_one_data(p, None)
            data['label'] = label
